...
void * web(void * data) {
    ...
    if(ret == 0 || ret == -1) { /* read failure stop now */
        ...
        len = (long)lseek(file_fd, (off_t)0, SEEK_END); /* 使用lseek来获得文件长度 */


        (void)lseek(file_fd, (off_t)0, SEEK_SET);

        // Socket write timing (header)
        (void)sprintf(buffer, "HTTP/1.1 200 OK\nServer: nweb/%d.0\nContent-Length: %ld\nConnection: close\nContent-Type: %s\n\n", VERSION, len, fstr); /* header + a blank line */
        logger(LOG, "header", buffer, hit);
        (void)write(fd, buffer, strlen(buffer));

        // File read and socket write timing
        while((ret = read(file_fd, buffer, BUFSIZE)) > 0) {


            (void)write(fd, buffer, ret);
        }
        usleep(10000); /* 在Socket通道关闭前,留出一段信息发送的时间 */
        close(file_fd);
    }
    close(fd);
    free(param); // 释放内存
...
int main(int argc, char **argv) {
    ...
    if((listenfd = socket(AF_INET, SOCK_STREAM, 0)) < 0)
        ...
        logger(ERROR,"Invalid port number(try1->60000)",argv[1],0);
    /*int opt = 1;
    if (setsockopt(listenfd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) < 0) {
        logger(ERROR, "setsockopt(SO_REUSEADDR) failed", "", 0);
    }
    ...
    for(hit = 1; ; hit++) {
        ...
        param->hit = hit;
        param->fd = socketfd;
        pthread_create(&pth, &attr, web, (void*)param);
